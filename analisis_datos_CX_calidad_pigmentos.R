...
acidos_grasos <- datos_limpios %>% 
  select(ID, provincia,cultivo_riego_secano,altitud,
         c40,c42,c44,acidez_percent,ip_mg_kg,k232,k270,
         carotenos_mg_kg,clorofilas_mg_kg
         )
...
nombres_acidos_grasos <- nombres_acidos_grasos[c(5:13)]
...
#carotenos_mg_kg: 1-2 (p = 0.0101894)
#clorofilas_mg_kg: 1-2 (p = 0.0373871)
...
#carotenos_mg_kg: 1-2 (p = 0.0436541)
...
#c44  : 0-1 (p = 0.004655766)
#acidez_percent : 0-2 (p = 0.04459142)
...
#c44  : 0-1 (p = 0.0070)
#acidez_percent : (p = 0.038909). No se puede hacer post hoc por interacciones
...
tabla_resumen_formato_final <- tabla_resumen_formato_final %>%
  mutate(Variable = c("C40","C42","C44","Acidez (%)","IP (mg/kg)",
                      "K232","K270","Carotenos (mg/kg)","Clorofilas (mg/kg)"))
