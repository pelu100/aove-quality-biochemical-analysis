...
acidos_grasos <- datos_limpios %>% 
  select(ID, provincia,cultivo_riego_secano,altitud,
        tocoferoles_totales,alfa_tocoferol,beta_tocoferol,
        gamma_tocoferol)
...
nombres_acidos_grasos <- nombres_acidos_grasos[c(5:8)]
...
#tocoferoles_totales: 0-2 (p = 0.014)
#alpha-tocoferol: 0-2 (p = 0.0083)
...
#tocoferoles_totales: 0-2 (p = 0.011)
#alpha-tocoferol: 0-2 (p = 0.0051)
...
tabla_resumen_formato_final <- tabla_resumen_formato_final %>%
  mutate(Variable = c("Tocoferoles totales","alpha-tocoferol","beta-tocoferol",
                      "gamma-tocoferol"))
