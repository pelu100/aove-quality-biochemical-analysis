...
acidos_grasos <- datos_limpios %>% 
  select(ID, provincia,cultivo_riego_secano,altitud,
         euvaol,eritrodiol,escualeno,
         acido_oleanolico,acido_ursolico,acido_maslinico)
...
nombres_acidos_grasos <- nombres_acidos_grasos[c(5:10)]
...
# En eritrodiol y escualeno hay una posible interaccion con el tipo de cultivo (p = 0.067 y p = 0.18)
...
# Euvaol: p = 0.037. No se ha podido realizar post hoc por posible interaccion con tipo cultivo (p = 0.18)
# Acido ursólico: 0-1 (p = 0.0016) y 0-2 (p = 0.022). Posible interaccion con tipo de cultivo (p = 0.13)

# Tipo de cultivo:
# Acido maslínico: p = 0.016. Interaccion con altitud (p = 0.022)
...
# Define cuántos decimales quieres en tu salida
decimales <- 2 # Puedes ajustar este número
...
tabla_resumen_formato_final <- tabla_resumen_formato_final %>%
  mutate(Variable = c("Euvaol","Eritrodiol","Escualeno",
                      "Acido oleanólico","Acido ursólico", "Acido maslínico"))
